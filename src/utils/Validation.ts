 ...
 */
export function validateChampionship(data: any): ValidationResult {
  const errors: string[] = [];
...
export function validateCategory(data: any): ValidationResult {
  ...
  if (data.teams && Array.isArray(data.teams)) {
    ...
    const uniqueTeams = new Set(data.teams.map((t: any) => (typeof t === 'string' ? t : t.name || '').trim().toLowerCase()));
 ...
 */
export function validateMatchResult(data: any): ValidationResult {
  const errors: string[] = [];
 ...
 */
export function validatePenalty(data: any): ValidationResult {
  const errors: string[] = [];
