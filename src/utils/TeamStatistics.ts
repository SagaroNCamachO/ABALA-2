...
export interface AdvancedTeamStats {
  ...
  winStreak: number; // Racha actual de victorias
  lossStreak: number; // Racha actual de derrotas
  bestWinStreak: number; // Mejor racha de victorias
  worstLossStreak: number; // Peor racha de derrotas
  averagePointsFor: number; // Promedio de puntos a favor
  averagePointsAgainst: number; // Promedio de puntos en contra
  averageDifference: number; // Promedio de diferencia
  biggestWin: { opponent: string; score: string; difference: number } | null;
  biggestLoss: { opponent: string; score: string; difference: number } | null;
  headToHead: Record<string, { wins: number; losses: number; draws: number }>; // Historial vs otros equipos
  recentForm: string[]; // Últimos 5 resultados (W/L)
  homeRecord: { wins: number; losses: number }; // Record como local
  awayRecord: { wins: number; losses: number }; // Record como visitante
}
...
export class TeamStatisticsCalculator {
   ...
   */
  static calculateAdvancedStats(
    team: Team,
    allMatches: Match[],
    categoryName: string
  ): AdvancedTeamStats {
    const teamMatches = allMatches.filter(
      m => (m.teamA === team.name || m.teamB === team.name) && m.played
    );
   ...
   */
  private static calculateStreaks(matches: Match[], teamName: string): {
    currentWin: number;
    currentLoss: number;
    bestWin: number;
    worstLoss: number;
    ...
    for (const match of sortedMatches) {
      ...
      if (won) {
        ...
        tempLoss++;
        tempWin = 0;
        if (tempLoss > worstLoss) worstLoss = tempLoss;
        // La racha actual es la primera que encontramos (más reciente)
        if (currentWin === 0 && currentLoss === 0) {
   ...
   */
  private static calculateAverages(team: Team, matches: Match[]): {
    pf: number;
    pc: number;
    diff: number;
  } {
   ...
   */
  private static calculateExtremes(matches: Match[], teamName: string): {
    biggestWin: { opponent: string; score: string; difference: number } | null;
    biggestLoss: { opponent: string; score: string; difference: number } | null;
  } {
    let biggestWin: { opponent: string; score: string; difference: number } | null = null;
    let biggestLoss: { opponent: string; score: string; difference: number } | null = null;

    for (const match of matches) {
      if (!match.scoreA || !match.scoreB || !match.winner) continue;
   ...
   */
  private static calculateRecentForm(matches: Match[], teamName: string): string[] {
    ...
    return sortedMatches.map(match => {
      ...
      if (match.winner && match.winner !== 'Empate') return 'L';
  ...
  private static calculateHomeAwayRecord(
    matches: Match[],
    teamName: string
  ): { home: { wins: number; losses: number }; away: { wins: number; losses: number } } {
