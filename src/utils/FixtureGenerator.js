...
exports.FixtureGenerator = void 0;
...
class FixtureGenerator {
     ...
     */
    static generateUniqueMatchups(teams) {
        const matchups = [];
     ...
     */
    static generateRoundRobin(teams, startRound, matchType = "ida") {
        if (teams.length < 2) {
            return [];
        }
        ...
        for (let i = 0; i < shuffled.length; i += 2) {
            ...
            const match1 = new Match_1.Match(matchup1[0], matchup1[1], roundNumber, matchType, matchday);
            match1.time = "20:00";
            matches.push(match1);
            // Crear segundo partido de la jornada (si existe)
            if (matchup2) {
                const match2 = new Match_1.Match(matchup2[0], matchup2[1], roundNumber, matchType, matchday);
                match2.time = "21:00";
                matches.push(match2);
            }
            matchday++;
     ...
     */
    static groupMatchupsIntoMatchdays(matchups) {
        const matchdays = [];
        const used = new Set();
        // Mezclar los enfrentamientos para distribución aleatoria
        ...
        for (let i = 0; i < shuffled.length; i++) {
            if (used.has(i))
                continue;
            const currentMatchday = [shuffled[i]];
            used.add(i);
